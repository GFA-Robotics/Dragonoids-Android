...
public class DragonoidsAuto extends LinearOpMode implements SensorEventListener {
    ...
    public void drive(Direction direction, int distance) throws InterruptedException {
        if (direction == Direction.Forward) {
            DragonoidsGlobal.setDrivePower(drivePower, drivePower, true);
        }
    ...
    public void driveTime(Direction direction, long milliseconds, double power) throws InterruptedException {
        if (direction == Direction.Forward) {
            DragonoidsGlobal.setDrivePower(power, power, true);
        }
        if (direction == Direction.Backward) {
            DragonoidsGlobal.setDrivePower(-power, -power, true);
        }
    ...
    public void autonomous(Alliance alliance) throws InterruptedException {
        ...
        DragonoidsGlobal.conveyor.setPower(-0.75);
        ...
        */
        ...
        currentTask = "Turn to beacon";
        this.turn(turnDirection, 45);
        // Drive forward to color detection distance
        currentTask = "Follow line";
        ...
        currentTask = "Press beacon button";
        ...
        */
        currentTask = "Reverse out";
        // Reverse out of the beacon area and prepare to climb mountain
        this.drive(Direction.Backward, 3000);
        this.turn(turnDirection, 45);
        this.driveTime(Direction.Forward, 5000);
