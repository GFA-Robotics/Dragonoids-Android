...
public class DragonoidsGlobal {
    ...
    //Make them not the same/ make direct
    public static void setDrivePower(double rightPower, double leftPower, boolean compensate, double forwardAmount) {
        final double compensation = 0.15;
        if (compensate) {
            rightPower += compensation;
        }
        //two is back set of motors
        rightOne.setPower(rightPower);
        rightTwo.setPower(forwardAmount);
        leftOne.setPower(leftPower);
        leftTwo.setPower(forwardAmount);
    }
